...
Kahana.InstanceOf = function ( obj, type )
{
	var type = (typeof type).toLowerCase();
	
	if( type != this.T_FUNC && type != this.T_OBJ ){
		throw Error( 'Kahana.InstanceOf( obj:object, type:[function|object]' );
	}
	
	return ( obj instanceof object && obj instanceof type );
};
...
Kahana.Iterate = function( origin, dest, callback, obj )
{
	if( origin == dest ){
		callback( dest, true, obj );
	}
	else
	{
		var checker;
		...
		if( origin < dest )
		{
			checker = function()
			{
				if( callback( origin++, false, obj ) || origin >= dest ){
					callback( dest, true, obj );
				}
				else {
