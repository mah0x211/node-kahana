...
**********************/
/* return object type */
Kahana.Inherits = function( class, super )
{
	var Traits = function(){};
	...
	for( var i = 0; i < classMethods.length; i++ ){
		class[classMethods[i]] = super[classMethods[i]];
	}
	/*
	for( var p in super )
	{
...
Kahana.prototype.Class = function( aObj )
{
	var ret = null;
...
Kahana.TypeOf = function()
{
	if( arguments.length % 2 ){
		throw Error( 'Kahana.TypeOf method parameter should be even number.' );
	}
...
Kahana.InstanceOf = function ( aObj, aType )
{
	var type = (typeof aType).toLowerCase();
	
	if( type != this.T_FUNC && type != this.T_OBJ ){
		throw Error( "Param Error : \n" + 'Kahana.InstanceOf( aObj:object, aType:[function|object]' );
	}
	
	return ( aObj instanceof Object && aObj instanceof aType );
};


Kahana.ObjectValForKey = function( aObj, aKey )
{
	var val = null;
	var finder = function( obj )
	{
		if( typeof obj == this.T_OBJ && aKey.length > 0 ){
			val = obj[aKey.shift()];
			finder( val );
		}
	};
...
module.exports = {
	Container: require( __dirname + '/Container' ),
	NotificationCenter: require( __dirname + '/NotificationCenter' )
};
