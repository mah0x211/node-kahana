...
Router.prototype.Run = function( uri, tick, ctx )
{
	var self = this,
		...
		execute = function( idx, last, wrap, runNext )
		{
			if( Kahana.TypeOf( self.delegator[wrap.methods[idx].method], Kahana.T_FUNC ) ){
				self.delegator[wrap.methods[idx].method]( wrap.uri, wrap.ctx, wrap.methods[idx].method, wrap.methods[idx].args, runNext );
			}
			else {
				self.delegator.Detour( wrap.uri, wrap.ctx, wrap.methods[idx].method, wrap.methods[idx].args, runNext );
			}
		};
	...
	{
		if( tick ){
			Kahana.IterateTick( 0, methods.length - 1, undefined, execute, { uri:uri, ctx:ctx, methods:methods } );
		}
		else{
			Kahana.Iterate( 0, methods.length - 1, undefined, execute, { uri:uri, ctx:ctx, methods:methods } );
		}
	}
	else {
