...
Router.prototype.Run = function( uri, r )
{
	var self = this,
		methods = this.Directive( uri ),
		execute = function( idx, last, ctx, next )
		{
			if( Kahana.TypeOf( self.delegator[ctx.methods[idx].method], Kahana.T_FUNC ) ){
				self.delegator[ctx.methods[idx].method]( ctx.uri, ctx.r, ctx.methods[idx].method, ctx.methods[idx].args, next );
			}
			else {
				self.delegator.Detour( ctx.uri, ctx.r, ctx.methods[idx].method, ctx.methods[idx].args, next );
			}
			// console.log( 'idx:' + idx + ' last:' + last );
			// next( ( last ) ? ctx : undefined );
		};
	...
	{
		if( r.data ){
			Kahana.IterateTick( 0, methods.length - 1, undefined, execute, { r:r, uri:uri, methods:methods } );
		}
		else{
			Kahana.Iterate( 0, methods.length - 1, undefined, execute, { r:r, uri:uri, methods:methods } );
		}
	}
	else {
		this.delegator.Detour( uri, r );
	}
};
