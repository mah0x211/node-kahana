...
var package = {
	...
	Auditor = require( __dirname + '/Auditor' );
...
function Server( app, confpath )
{
	...
	// set event
	if( typeof app['ParseError'] === 'function' )
	{
		this.evt.on( 'parse_error', function( err ){
			app['ParseError']( err );
		} );
	}
};
...
Server.prototype.Configure = function()
{
	...
	// set DelegateRendering
	if( String( this.conf.DelegateRendering ).toLowerCase() == 'on' ){
		this.conf.DelegateRendering = 1;
		this.generator = this.app;
	}
	else {
		this.conf.DelegateRendering = 0;
		// page generator
		this.generator = new Kahana.ClearSilver;
	}
	// set EnableInclude: clearsilver include function
	if( String( this.conf.EnableInclude ).toLowerCase() == 'on' ){
		this.generator.SetDocumentRoot( this.conf.DocumentRoot );
	}
	// set TemplateAs
	...
	if( this.conf.Audit )
	{
		for( var p in this.conf.Audit ){
			this.auditor.CondAdd( p, this.conf.Audit[p] );
		}
	}
...
Server.prototype.RequestClose = function( r, status )
{
	...
	else
	{
		...
		else
		{
			...
			if( this.conf.TemplateAs.test( r.parsed_url.pathfile ) )
			{
				// try to rendering
				try {
				...
				// set content-length
				r.headers['Content-Length'] = r.page.length;
			}
		}
