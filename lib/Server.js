...
Server.prototype.ErrorPage = function( r )
{
	...
	switch( r.status )
	{
		...
		break;
		case 405:
			r.page = '<html><head><title>405 METHOD NOT_ALLOWED</title></head><body><h1>405 METHOD NOT_ALLOWED</h1><p>Failed to Requested URL: ' + encodeURI( r.req.url ) + '<br />reason: METHOD NOT_ALLOWED </p></body></html>';
		break;
		case 406:
		...
		break;
		case 407:
			r.page = '<html><head><title>407 PROXY AUTHENTICATION_REQUIRED</title></head><body><h1>407 PROXY AUTHENTICATION_REQUIRED</h1><p>Failed to Requested URL: ' + encodeURI( r.req.url ) + '<br />reason: PROXY AUTHENTICATION_REQUIRED </p></body></html>';
		break;
		case 408:
			r.page = '<html><head><title>408 REQUEST TIME_OUT</title></head><body><h1>408 REQUEST TIME_OUT</h1><p>Failed to Requested URL: ' + encodeURI( r.req.url ) + '<br />reason: REQUEST TIME_OUT </p></body></html>';
		break;
		case 409:
		...
		break;
		case 413:
			r.page = '<html><head><title>413 REQUEST ENTITY_TOO_LARGE</title></head><body><h1>413 REQUEST ENTITY_TOO_LARGE</h1><p>Failed to Requested URL: ' + encodeURI( r.req.url ) + '<br />reason: REQUEST ENTITY_TOO_LARGE </p></body></html>';
		break;
		case 414:
			r.page = '<html><head><title>414 REQUEST URI_TOO_LARGE</title></head><body><h1>414 REQUEST URI_TOO_LARGE</h1><p>Failed to Requested URL: ' + encodeURI( r.req.url ) + '<br />reason: REQUEST URI_TOO_LARGE </p></body></html>';
		break;
		case 415:
			r.page = '<html><head><title>415 UNSUPPORTED MEDIA_TYPE</title></head><body><h1>415 UNSUPPORTED MEDIA_TYPE</h1><p>Failed to Requested URL: ' + encodeURI( r.req.url ) + '<br />reason: UNSUPPORTED MEDIA_TYPE </p></body></html>';
		break;
		case 416:
			r.page = '<html><head><title>416 RANGE NOT_SATISFIABLE</title></head><body><h1>416 RANGE NOT_SATISFIABLE</h1><p>Failed to Requested URL: ' + encodeURI( r.req.url ) + '<br />reason: RANGE NOT_SATISFIABLE </p></body></html>';
		break;
		case 417:
		...
		break;
		case 504:
			r.page = '<html><head><title>504 GATEWAY TIME_OUT</title></head><body><h1>504 GATEWAY TIME_OUT</h1><p>Failed to Requested URL: ' + encodeURI( r.req.url ) + '<br />reason: GATEWAY TIME_OUT </p></body></html>';
		break;
		case 505:
			r.page = '<html><head><title>505 VERSION NOT_SUPPORTED</title></head><body><h1>505 VERSION NOT_SUPPORTED</h1><p>Failed to Requested URL: ' + encodeURI( r.req.url ) + '<br />reason: VERSION NOT_SUPPORTED </p></body></html>';
		break;
		case 506:
			r.page = '<html><head><title>506 VARIANT ALSO_VARIES</title></head><body><h1>506 VARIANT ALSO_VARIES</h1><p>Failed to Requested URL: ' + encodeURI( r.req.url ) + '<br />reason: VARIANT ALSO_VARIES </p></body></html>';
		break;
		case 507:
		...
		default:
			r.status = 500;
			r.page = '<html><head><title>500 INTERNAL SERVER_ERROR</title></head><body><h1>500 INTERNAL SERVER_ERROR</h1><p>Failed to Requested URL: ' + encodeURI( r.req.url ) + '<br />reason: INTERNAL SERVER_ERROR </p></body></html>';
		break;
	}
}

Server.SetHeader = function( r, key, val ){
	r.headers[key] = val;
};

Server.AddHeader = function( r, key, val )
{
	var tmp = r.headers[key];
	
	if( tmp )
	{
		if( Kahana.InstanceOf( tmp, Array ) ){
			tmp.push( val );
		}
		else {
			r.headers[key] = [tmp,val];
		}
	}
	else {
		r.head[key] = val;
...
Server.prototype.RequestData = function( r, callback )
{
			...
			switch( err.errno ){
				...
				case 21:	// EISDIR
					...
					if( !r.parsed_url.pathname_resolve && r.server.conf.DirectorySlash ){
						Server.SetHeader( r, 'Location', r.parsed_url.pathname + '/' + 
	...
	package.fs.stat( r.parsed_url.pathfile, function( err, stats )
	{
		...
		else if( stats.isDirectory() && !r.parsed_url.pathname_resolve )
		{
			...
			if( r.server.conf.DirectorySlash )
			{
				...
				Server.SetHeader( r, 'Location', url );
				status = 301;
			}
			// not found
			else {
