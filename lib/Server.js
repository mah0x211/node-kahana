...
function Server( app, confpath )
{
	...
	this.defaultConf = {
		...
		// EnableInclude: undefined,
		// Audit: undefined,
		Route: {}
	};
	this.app = app;
	// event emitter
	this.evt = new package.EventEmitter();
...
Server.prototype.Configure = function()
{
	...
	// set Timeout
	if( typeof this.conf.Timeout === 'number' ){
		this.conf.Timeout = +this.conf.Timeout * 1000;
	}
	// set KeepAlive
	this.conf.KeepAlive = ( String( this.conf.KeepAlive ).toLowerCase() == 'on' ) ? true : false;
	// set DirectorySlash
	...
	if( typeof this.app.SetDocumentRoot === 'function' ){
		this.app.SetDocumentRoot( this.conf.DocumentRoot );
	}
	// set TemplateAs
	this.conf.TemplateAs = new RegExp( '\\.(' + this.conf.TemplateAs.join('|') + ')$' );
	// check Session cookie format
	if( this.conf.Session )
	{
		// secret
		if( this.conf.Session.secret && typeof this.conf.Session.secret !== 'string' ){
			throw Error( 'invalid configuration: Sesson.secret must be set string' );
		}
		// name
		if( typeof this.conf.Session.name !== 'string' ){
			throw Error( 'invalid configuration: Sesson.name must be set string' );
		}
		// domain
		if( this.conf.Session.domain && typeof this.conf.Session.domain !== 'string' ){
			throw Error( 'invalid configuration: Sesson.domain must be string' );
		}
		// port
		if( this.conf.Session.port && typeof this.conf.Session.port !== 'number' ){
			throw Error( 'invalid configuration: Sesson.port must be number' );
		}
		// path
		if( this.conf.Session.path && typeof this.conf.Session.path !== 'string' ){
			throw Error( 'invalid configuration: Sesson.path must be string' );
		}
		// expire
		if( this.conf.Session.expire )
		{
			if( typeof this.conf.Session.expire !== 'number' ){
				throw Error( 'invalid configuration: Sesson.expire must be number' );
			}
			else {
				Session.expire = this.conf.Session.expire;
			}
		}
		else {
			this.conf.Session.expire = Session.expire;
		}
	}
	// create pid file and write process-id
	// set PidFile
	if( this.conf.PidFile )
	{
		var self = this;
		package.fs.realpath( this.conf.PidFile, function( err, resolve )
		{
			var pidfd;
			
			if( err ){
				self.conf.PidFile = err.path;
			}
			else {
				self.conf.PidFile = resolve;
			}
			pidfd = package.fs.openSync( self.conf.PidFile, 'w' );
	...
	this.router.Set( this.conf.Route );
};
...
Server.prototype.RequestOpen = function( req, res )
{
	...
	else
	{
		...
		this.nconn++;
		// set keep-alive off
		res.shouldKeepAlive = this.conf.KeepAlive;
		// set Timeout event
		...
		{
			// parse cookie
			this.RequestCookie( r );
			// check session cookie
			if( this.conf.Session )
			{
				var session = ( r.cookie && r.cookie[this.conf.Session.name] ) ? 
								r.cookie[this.conf.Session.name][0] : 
								undefined;
				
				if( session ){
					r.session = Session.varify( session, true );
				}
			}
...
Server.prototype.RequestClose = function( r, status )
{
	...
	else
	{
		...
		{
			// set by filename
			if( r.parsed_url.pathfile ){
				r.mime = package.mime.lookup( r.parsed_url.pathfile );
			}
			// default type
			else {
				r.mime = this.conf.DefaultType;
			}
		}
		
		// rendering if match is template file
		// try to rendering
		this.app.Render( r, this.conf.TemplateAs.test( r.parsed_url.pathfile ), function( err, r )
		{
			if( err ){
				r.status = 500;
				self.ErrorPage( r );
			...
			// set session cookie
			if( self.conf.Session && r.session )
			{
				var cookie = {
					name: self.conf.Session.name, 
					val: r.session.cookie, 
					expires: r.session.exp.toUTCString(), 
				};
				
				if( self.conf.Session.domain ){
					cookie.domain = self.conf.Session.domain;
				}
				if( self.conf.Session.port ){
					cookie.port = self.conf.Session.port;
				}
				if( self.conf.Session.path ){
					cookie.path = self.conf.Session.path;
				}
				// set cookie: session
				self.SetCookie( r, cookie, true );				
			}
			// set content-length
			r.headers['Content-Length'] = ( Buffer.isBuffer( r.page ) ) ? r.page.length : Buffer.byteLength( r.page );
			// set content-type
...
Server.prototype.RequestData = function( r, progress, callback )
{
	if( r.status == 0 )
	{
			...
			!r.req.headers['content-type'].match( '^(application/x-www-form-urlencoded|multipart/form-data)' ) )
		{
			...
			if( !r.data ){ r.data = { length: 0, fields:{}, files:{} }; }
...
Server.prototype.SetHeader = function( r, key, val, add )
{
	if( add )
	{
		if( !r.headers[key] ){
		...
		else if( r.headers[key].constructor !== Array ){
...
Server.prototype.PutHeader = function( r )
{
	for( var p in r.headers ){
		r.res.setHeader( p, r.headers[p] );
	}
};

Server.prototype.SessionCreate = function( r )
{
	// check session cookie
	if( this.conf.Session && !r.session ){
		r.session = Session.create();
		return true;
	}
	
	return false;
}
Server.prototype.SessionUpdate = function( r )
{
	// if define session conf
	if( this.conf.Session )
	{
		var cookie;
		
		r.session = Session.update( r.session );
		// set session-cookie
		if( !r.session ){
			r.session = Session.create();
		}
		...
		cookie = { 
			name: this.conf.Session.name, 
			val: r.session.cookie, 
			expires: r.session.exp.toUTCString(), 
		};
		if( this.conf.Session.domain ){
			cookie.domain = this.conf.Session.domain;
		}
		if( this.conf.Session.port ){
			cookie.port = this.conf.Session.port;
		}
		if( this.conf.Session.path ){
			cookie.path = this.conf.Session.path;
		}
		// set cookie: session
		this.SetCookie( r, cookie, true );
...
Server.prototype.SetCookie = function( r, cookie, add )
{
	if( cookie.name )
	{
		var fields = ['expires','domain','path','port'],
			val = [cookie.name + '=' + escape(cookie.val)];
		
		fields.forEach( function( field )
		{
			if( cookie[field] ){
				val.push( field + '=' + cookie[field] );
			}
		});
		val = val.join('; ') + '; ';
		if( cookie.secure ){
		...
		this.SetHeader( r, 'Set-Cookie', val, add );
	}
};
...
Server.prototype.TranslateName = function( uri )
{
	...
	// parse url
	parsed_url = package.url.parse( uri, true );
	// path normalize
